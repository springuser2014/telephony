...
function(App, $, Mustache, cookies, rest, auth) {
	...
	var getCurrentFormFilterValues = function() {
		...
		var obj = {
			...
			saleDateEnd : ($('#date_to').val().length > 0 ? $('#date_to').val() : null),
			saleDateStart : ($('#date_from').val().length > 0 ? $('#date_from').val() : null),
	...
	var fetchAndUpdateView = function() {
		...
		Telephony.Rest.Sales.Fetch(
			...
			function(jqXHR, status) {
				...
				for (var i = 0; i < params1.sales.length; i++) {
					var s = params1.sales[i];

					var date = new Date();
					date.setTime(s.dateOut);
				...
				var template = $('#sale-list-template').html();
	...
	var addSaleView = function() {
		var tmpl = $('#sale-add-template').html();
	...
	var createFiltersView = function() {
		...
		var template = $('#sale-search-form-template').html();
